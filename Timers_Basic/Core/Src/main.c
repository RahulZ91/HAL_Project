...
int main(void)
{
	...
	// which occurs when the ARR count reaches the max value
	while(!(TIM6->SR & TIM_SR_UIF)); // loops till bit is not set ie 100ms have not elapsed
	// once 100ms elapse the LED is toggles
	TIM6->SR=0;
