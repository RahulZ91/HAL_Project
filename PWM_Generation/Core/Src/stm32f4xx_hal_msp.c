...
{
  // Low level inits
	GPIO_InitTypeDef tim2oc_ch;
 // Clock inits
	__HAL_RCC_TIM2_CLK_ENABLE();
	__HAL_RCC_GPIOA_CLK_ENABLE(); // for the port a
	__HAL_RCC_GPIOB_CLK_ENABLE();// for the port b
	// configure the GPIO pins for channel 1,2,3 and 4
	//PA0 - channel 1
	//PA1 - channel 2
	//PB10 - channel 3
	//PB2 - channel 4
	tim2oc_ch.Pin=GPIO_PIN_0 | GPIO_PIN_1;
	tim2oc_ch.Mode=GPIO_MODE_AF_PP;
	tim2oc_ch.Alternate= GPIO_AF1_TIM2;
	tim2oc_ch.Pull=GPIO_NOPULL;
	tim2oc_ch.Speed=GPIO_SPEED_FREQ_LOW;

	HAL_GPIO_Init(GPIOA, &tim2oc_ch);

	tim2oc_ch.Pin=GPIO_PIN_2 | GPIO_PIN_10;
	tim2oc_ch.Mode=GPIO_MODE_AF_PP;
	tim2oc_ch.Alternate= GPIO_AF1_TIM2;
	tim2oc_ch.Pull=GPIO_NOPULL;
	tim2oc_ch.Speed=GPIO_SPEED_FREQ_LOW;

	HAL_GPIO_Init(GPIOB, &tim2oc_ch);
