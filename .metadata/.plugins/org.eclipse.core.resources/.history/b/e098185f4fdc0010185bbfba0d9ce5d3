...
void TIMER2_Init(void)
{
	...
	htimer2.Init.Period=0xFFFFFFFF; // max count for 32 bit
	htimer2.Init.Prescaler=1;
	if(HAL_TIM_OC_Init(&htimer2) != HAL_OK)  // init in OC
	{
		while(1);
	}

	// OC mode
	tim2_oc_init.OCMode=TIM_OCMODE_TOGGLE; // since we want the signal to toggle based on counter value
	tim2_oc_init.OCPolarity=TIM_OCPOLARITY_HIGH; // this is for the mux ccp1 - high is 0, low is 1
	tim2_oc_init.Pulse=pulse_one_value;
	if(HAL_TIM_OC_ConfigChannel(&htimer2, &tim2_oc_init,TIM_CHANNEL_1) !=HAL_OK)
	{
		while(1);
	}
	tim2_oc_init.Pulse=pulse_two_value;
	if(HAL_TIM_OC_ConfigChannel(&htimer2, &tim2_oc_init,TIM_CHANNEL_2) !=HAL_OK)
	{
		while(1);
	}
	tim2_oc_init.Pulse=pulse_three_value;
	if(HAL_TIM_OC_ConfigChannel(&htimer2, &tim2_oc_init,TIM_CHANNEL_3) !=HAL_OK)
	{
		while(1);
	}
	tim2_oc_init.Pulse=pulse_four_value;
